...
### `src/core/stream_tracker.py` — Multi-Monitor Distribution

`GlobalVideoAssigner` manages stream URL distribution across multiple monitor instances, trying to give each monitor a non-overlapping set of streams. It's instantiated by `DisplayManager` but the primary assignment logic runs in `VideoManager` — the tracker is used as a supplementary helper.
...
- `file_utils.py` — parses `m3u8-hosts.m3u8` (skips comments, deduplicates, basic URL validation) and recursively scans folders for video files (`.mp4`, `.mkv`, `.avi`, `.mov`, `.wmv`, `.flv`, `.webm`, `.m4v`, `.mpg`, `.mpeg`, `.3gp`)
- `stream_utils.py` — HTTP HEAD-based stream validation and exponential backoff calculation (utility functions, not called in the main playback loop)
