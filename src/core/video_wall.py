...
from src.config.settings import STREAM_CHECK_INTERVAL_MS
...
class VideoWall(QMainWindow):
    ...
    def _setup_refresh_timer(self):
        ...
        self.stream_check_timer.start(STREAM_CHECK_INTERVAL_MS)
    ...
    def check_stream_health(self):
        ...
        for i, player in enumerate(self.video_manager.players):
            ...
            if self.display_manager.tiles[
                i
            ].isVisible() and not self.video_manager.using_local_video.get(i, True):
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_Escape:
            ...
            # R key toggles screen recording
            state = self.recorder.toggle()
            if state:
                print("Recording: ON")
            else:
                print("Recording: OFF")
    ...
    def closeEvent(self, event):
        ...
        # Clean up players — stop loading timers, pause, and release media
        if hasattr(self, "video_manager"):
            for player in self.video_manager.players:
                if hasattr(player, "_loading_timer"):
                    player._loading_timer.stop()
                    delattr(player, "_loading_timer")
                player.stop()
                player.setMedia(QMediaPlayer.NullMedia)
            self.video_manager.players.clear()
